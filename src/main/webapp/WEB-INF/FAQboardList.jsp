...
<%
	...
	count = fbdao.getAllcount();//전체 게시글의 숫자를 파악하여 저장
	...
	Vector<FAQboardBean> vec = fbdao.allselectboard(startRow,endRow);
	Vector<FAQboardBean> Nvec = fbdao.noticeboardselect();//공지글
...
<table class="board-table" style="align:center; width:100%; ">
	...
	for(int i=0; i<Nvec.size();i++){
		FAQboardBean fbBean = Nvec.get(i);
	%>	
	<tr height="40"  align="center" style="background: #EFF8FB">
		...
		<td align="left" width="140">
			...
			<%= fbBean.getFb_title()  %></a></td>
		
		<td>
			<a href="main.jsp?center=FAQboardSearchList.jsp?WorT=writer&what=<%=fbBean.getFb_writer()%>"style="text-decoration:none;  color: inherit;">
	...
	for(int i=0; i<vec.size();i++){
		...
		FAQboardBean fbBean = vec.get(i);
		if(fbBean.getFb_notice()==1)
			continue;
	%>
	...
	<nav aria-label="Page navigation example"  style=" position: relative;left: 5%;bottom: 20px;width: 500px;margin-right: 500px;">
		...
		<%
			if(count > 0){
				...
				if(startPage > 10){
		%>
					<li class="page-item"><a class="page-link" href="main.jsp?center=FAQboardList.jsp?pageNum=<%=startPage - 10%>" style="text-decoration:none; color: inherit;">[previous]</a></li>			
		<%
			
				}
				for(int i = startPage; i <= endPage; i ++){
				%>
					<li class="page-item"><a class="page-link" href="main.jsp?center=FAQboardList.jsp?pageNum=<%=i%>" style="text-decoration:none; color: inherit;"><%=i %></a></li>
				<%	
				}
				//[다음]링크를 만드는 부분
				if(endPage < pagecount){
				...
				%>
					<li class="page-item"><a class="page-link" href="main.jsp?center=FAQboardList.jsp?pageNum=<%=startPage + 10%>" style="text-decoration:none; color: inherit;">[next]</a></li>
				<% 
				}
			}
		%>
		...
		<div id="board-search" style="width: 100%;">
	        <div class="container" style="width: 100%;margin-left: 0px;padding-left: 0px;padding-right: 0px;margin-right: 0px;">
